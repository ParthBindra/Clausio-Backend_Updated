...
public class AiService(ClausioDbContext db, IAiClient aiClient) : IAiService
{
        ...
        string? customSystemPrompt = null)
    {
        ...
        // ✅ Get case type for specific legal context
        var caseEntity = await db.Cases
            .AsNoTracking()
            .FirstOrDefaultAsync(c => c.Id == caseId, cancellationToken);

        var caseTypePrompt = CaseTypePrompts.GetSystemPrompt(caseEntity?.CaseType);

        var systemPrompt = (customSystemPrompt ?? BaseSystemPrompt)
            + "\n\n"
            + caseTypePrompt
            + "\n\n"
            + dossier;
        ...
        if (c.Documents.Count > 0)
        {
            ...
            foreach (var d in c.Documents)
            {
                ...
                sb.AppendLine("    Type: " + d.DocumentType +
                             ...
                             " | Size: " + (d.SizeBytes > 0 ? Math.Round(d.SizeBytes / 1024.0, 1) + " KB" : "Unknown"));
                if (false)
        ...
        if (c.Hearings.Count > 0)
        {
            ...
            foreach (var h in c.Hearings.OrderByDescending(x => x.HearingDate))
            {
                ...
                foreach (var o in h.Orders)
                {
                    ...
                    sb.AppendLine("    Due: " + (o.Deadline.ToString("dd MMM yyyy")) +
